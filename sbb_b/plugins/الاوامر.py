from .. import *


@sbb_b.ar_cmd(pattern="الاوامر")
async def hi(event):
    await event.reply(
        "𓆩𝙏𝞝𝙇𝞝𝙃𝙊𝙉 𝙎𝙋𝙄𝘿𝞝𝙍 \n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n\n**⎊ هـذه قـائـمـة اوامـر سـورس سـبايـدر **\n\n⎊ `.الامر1` ◂ اوامر الأدمن\n⎊ `.الامر2` ◂ اوامر المجموعة\n⎊ `.الامر3` ◂ اوامر الترحيب\n⎊ `.م4` ◂ اوامر الردود\n⎊ `.م5` ◂ اوامر الترفيه\n⎊ `.م6` ◂ اوامر حماية الخاص\n⎊ `.م7` ◂ اوامر الاذاعه و الكشف \n⎊ `.م8` ◂ اوامر البوت \n⎊ `.م9` ◂ اوامر المنع والترجمه\n⎊ `.م10` ◂ اوامر التلكراف و النطق\n⎊ `.م11` ◂ اوامر التحميل\n⎊ `.م12` ◂ اوامر التكرار\n⎊ `.م13` ◂ اوامر الانتحال والتقليد\n⎊ `.م14` ◂ اوامر الوقتي \n⎊ `.م15` ◂ اوامر الذاتيه والاضافه\n⎊ `.م16` ◂ اوامر البروفايل\n⎊ `.م17` ◂ اوامر الأكسترا\n⎊ `.م18` ◂ التاك و الملفات\n⎊ `.م19` ◂  اوامر الصيغ\n⎊ `.م20` ◂  اوامر التسلية\n⎊ `.م21` ◂  اوامر الفارات\n⎊ `.م22` ◂  اوامـر التنظيـف\n⎊ `.م23` ◂  اوامـر المسـح\n⎊ `.م24` ◂  اوامـر القفل والفتح\n⎊ `.م25` ◂  اوامر الهمسه او واكس\n⎊ `.م26` ◂  بصمات الانمي\n⎊ `.م27` ◂  اوامر الروابط\n⎊ `.م28` ◂  اوامر التحكم\n⎊ `.م29` ◂  تجميع النقاط\n⎊ `.م30` ◂ اضافات سورس سبايدر \n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ⌯︙[ ┈┉━｢ ᥉᥆υᖇᥴᥱ ᥉ρᎥժᥱᖇ 🕷 ｣━┅┈ ](t.me/EE_20)",
        link_preview=False,
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="الامر1")
async def hi(event):
    await event.reply(
        "قائمة اوامر الادمن لسورس سبايدر  :\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n◂  `.حظر` \n❃ لحظر الشخص في مجموعه بالرد عليه\n\n ◂  `.الصورة -حذف`\n❃ اكتب الامر في المجموعة لحذف صورتها\n\n ◂  `.الصورة -وضع`\n❃ بالرد على الصورة في المجموعة لوضعها صورة للكروب\n\n ◂  `.الغاء الحظر`\n❃ لالغاء حظر الشخص من المجموعه بالرد عليه\n\n ◂  `.كتم` \n❃ لكتم الشخص بالرد عليه او وضع معرفه مع الامر\n\n ◂  `.الغاء كتم`\n❃ لالغاء كتم الشخص بالرد عليه بالامر\n\n ◂  `.تثبيت` \n❃ بالرد على الرسالة لتثبيتها في المجموعة\n\n ◂  `.الغاء تثبيت` \n❃ لألغاء تثبيت الرسالة في المجموعة , اذا اردت الغاء تثبيت جميع الرسائل اكتب مع الامر الكل\n\n ◂  `.رفع مشرف`  < لقب > \n❃ بالرد ؏ المستخدم لرفعه مشرف يمكنك وضع لقب\n\n ◂  `.تنزيل مشرف` \n❃ بالرد ؏ المستخدم لتنزيله من المشرفين\n\n ◂  `.ارفع`\n❃ بالرد ؏ المستخدم لرفعه مشرف في جميع المجموعات \n\n ◂  `.نزل`\n❃ بالرد ؏ المستخدم لتنزيله من جميع المجموعات\n\n ◂  `.الاحداث`\n❃ فقط ارسل الامر لعرض اخر احداث المجموعه \n\n ◂  `.الغاء المحظورين`\n❃ لالغاء جميع المحظورين في المجموعه فقط اكتب الامر\n\n ◂  `.المحذوفين`\n❃ لعرض الحسابات المحذوفة في المجموعه فقط اكتب الامر\n\n ◂  `.تفليش`\n❃ لحظر جميع المستخدمين من المجموعه فقط اكتب الامر\n\n ◂  `.تنزيل الكل`\n❃ لتنزيل جميع المشرفين من المجموعه فقط ارسل الامر في المجموعه\n\n ◂  `.المحظورين`\n❃ لعرض المستخدمين المحظورين في المجموعه اكتب الامر فقط\n\n ◂  `.تحذير`\n❃ بالرد على المستخدم لتحذيره في المجموعه \n\n ◂  `.حذف التحذيرات`\nلحذف تحذيرات المستخدم بالرد عليه او وضع معرفه مع الامر\n\n ◂  `.التحذيرات`\n❃ بالرد على المستخدم لعرض عدد تحذيراته\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="الامر2")
async def hi(event):
    await event.reply(
        "** قائمة اوامر المجـموعه لسورس سبايدر **:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.المشرفين` \n❃ فقط اكتب الامر في المجموعه لعرض مشرفين المجموعه\n\n ◂  `.البوتات` \n❃ فقط اكتب الامر في الدردشة لعرض البوتات المضافة \n\n ◂  `.الاعضاء`\n❃ فقط اكتب الامر في المجموعه لعرض أعضاء الدردشة\n\n ◂  `.معلومات`\n❃ لعرض معلومات المجموعه اكتبه الامر في المجموعة\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="الامر3")
async def hi(event):
    await event.reply(
        "** قائمة اوامر الـترحيب **:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.ترحيب`\n❃ اكتب الامر مع الترحيب ليقوم بالترحيب في المستخدمين الجدد في المجموعة\n\n ◂  `.الترحيب`\n❃ لعرض رساله الترحيب الحاليه اكتب الامر فقط\n\n ◂  `.حذف الترحيب`\n❃ لالغاء الترحيب في المستخدمين فقط اكتب الامر\n\n⌔∮ متغيرات الترحيب [اضغط هنا](https://t.me/X_koo)\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20",
        link_preview=False,
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م4")
async def hi(event):
    await event.reply(
        "** قائمة اوامر  الردود **:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.رد`\n❃ يستخدم الأمر بإضافة رد في المجموعه اكتب الامر والرد الخاص بك بالرد على الكلمه \n\n ◂  `.حذف الردود`\n❃ فقط اكتب الامر في الدردشه لحذف جميع الردود المضافه\n\n ◂  `.الردود`\n❃ لعرض الردود المصافه فقط اكتب الامر\n\n ◂  `.ايقاف`\n❃ اكتب الامر مع الرد لحذف الرد من الدردشه\n\n⌔∮ شرح توضيحي عن اوامر الردود  [اضغط هنا](https://t.me/X_koo)\n⌔∮ شرح عن متغيرات الردود  [اضغط هنا](https://t.me/X_koo)\n𓍹—————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20",
        link_preview=False,
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م5")
async def hi(event):
    await event.reply(
        "** قائمة اوامر الرفع **:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n• ◂  `.رفع مطي`\n ◂  `.رفع كلب`\n ◂  `.رفع حيوان`\n ◂  `.رفع زاحف`\n ◂  `.رفع مرتي`\n ◂  `.رفع زوجي`\n ◂  `.رفع تاج`\n ◂  `.رفع بكلبي`\n ◂  `.رفع بزون`\n ◂  `.رفع قرد`\n\n ◂  `.نسبة الحب`\n ◂  `.نسبة الانوثة`\n ◂  `.نسبة الرجولة`\n ◂  `.نسبة الغباء`\n ◂  `.نسبة المثْلية`\n\n ◂  `.كت`\n ◂  `.اوصف`\n ◂  `.هينه`\n ◂  `.نزوج`\n ◂  `.طلاك`\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م6")
async def hi(event):
    await event.reply(
        "** قائمة اوامر حـماية الخاص والتلكراف **:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n◂  `.الحماية تشغيل`\n❃ لتشغيل امر الحمايه يستخدم لتحذير المستخدمين عند مراسلتك في الخاص من التكرار\n\n ◂  `.الحماية تعطيل`\n❃ لتعطيل امر الحمايه وعدم تحذير المستخدمين في الخاص\n\n ◂  `.سماح` او `.س`\n❃ يستخدم الامر لقبول الشخص في الخاص وعدم ارسال تحذيرات له بالرد على الشخص\n\n ◂  `.رفض` او `.ر`\n❃ يستخدم لرفض الشخص من الخاص وتحذيره اذا كرر الرسائل وبعدها حظره\n\n ◂  `.بلوك`\n❃ بالرد على المستخدم لحظره من الخاص\n\n ◂  `.انبلوك`\n❃ بالرد على المستخدم لالغاء حظره من الخاص\n\n ◂  `.المسموح لهم`\n❃ اكتب الامر فقط لعرض معلومات عن الاشخاص الذين قبلته في الخاص\n\n⌔∮ لعرض فارات الحماية ارسل `.الفارات`\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯—————𓍻\n⌔︙CH : @EE_20",
        link_preview=False,
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م7")
async def hi(event):
    await event.reply(
        "**قائمـه اوامر الكشف والاذاعة**:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.ايدي`\n❃ بالرد على المستخدم او وضع معرفه مع الامر لعرض معلوماته\n\n ◂  `.كشف`\n❃ بالرد على المستخدم لعرض معلومات معينه عنه\n\n ◂  `.للخاص`\n❃ اكتب الامر بالرد على رساله او اي ميديا ليقوم بارساله لجميع المحادثات في الخاص\n\n ◂  `.للكروبات`\n❃ بالرد على نص او اي ميديا ليقوم بنشره في جمبع المجموعات\n\n⌔∮ متغيرات الايدي  [اضغط هنا](https://t.me/X_koo)\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯—————𓍻\n⌔︙CH : @EE_20",
        link_preview=False,
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م8")
async def hi(event):
    await event.reply(
        "**قائمـه اوامر البوت**:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.فحص`\n❃ فقط اكتب الامر لعرض معلومات السورس\n\n ◂  `.بنك`\n❃ فقط اكتب الامر لعرض سرعه البوت\n\n ◂  `.الانترنت + الاضافه`\n❃ يستخدم الامر لقياس سرعه البوت اكتب الامر مع الاضافه  :  `.الانترنت صورة`  `.الانترنت نص`\n\n ◂  `.اعادة تشغيل`\n❃ لعمل اعادة تشغيل للسورس وتحديثه \n\n ◂  `.الاشعارات` + تشغيل/تعطيل\n❃ لتشغيل او تعطيل الاشعارات عند تحديث او اعادة تشغيل السورس\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م9")
async def hi(event):
    await event.reply(
        "**قائمـه اوامر المنع والترجمة** :\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.منع`\n❃ اكتب الاكر مع الكلمه لمنع ارساله في المجموعه او اي دردشه\n\n ◂  `.الغاء منع`\n❃ اكتب الامر مع الكلمه لالغاء منعها من الدردشه\n\n ◂  `.قائمة المنع`\n❃ اكتب الامر لعرض قائمه الكلمات التي منعتها في الدردشة\n\n ◂  `.ترجمة`\n❃ يستخدم الاكر لترجمه الكلمات اكتب الامر مع كود اللغه بالرد غلى النص لترجمته\n\n⌔∮ اكواد اللغات للترجمه  [اضغط هنا](https://telegra.ph/SOURCE-SPIDER-03-08)\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯—————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م10")
async def hi(event):
    await event.reply(
        "**قائمـه اوامر التلكراف و النطق** :\n 𓍹—————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.انطق`\n❃ بالرد على الكلام لتسجيله ببصمه و ارساله لك\n\n ◂  `.تلكراف ميديا` ◂  \n❃ بالرد على الميديا لصنع رابط تلكراف منها\n\n ◂  `.تلكراف نص`\n❃ بالرد على النص لعمل رابط تلكراف\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م11")
async def hi(event):
    await event.reply(
        "**قائمـه اوامر التحميل** :\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.متحركات`\n❃ بكتابة عنوان مع الامر لجلب متحركة لك من العنوان\n\n ◂  `.فلم`\n❃ بكتابة اسم الفلم باللغة الانجليزية مع الامر لعرض معلومات الفلم\n\n ◂  `.تحميل فيديو`\n❃ بالرد على رابط من يوتيوب او اي موقع ثاني لتحميل الفيديو وارساله لك\n\n ◂  `.تحميل صوتي`\n❃ اكتب الامر بالرد على الرابط من اليوتيوب لتكميل مقطع الصوتي و ارساله لك\n\n ◂  `.انستا`\n❃ اكتب الامر بالرد على الرابط التحميل من الانستا \n\n ◂  `.بينترست`\n❃ بالرد على رابط من بينتسرت للتحميل لك\n\n ◂  `.صور`\n❃ اكتب الامر مع عنوان للبحث عنه وارسال لك الصور،  اذا بحثت عن اباحي سيتم تعطيل حسابك\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م12")
async def hi(event):
    await event.reply(
        "** قائـمه اوامر التكرار**:\n 𓍹—————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n◂ `.كرر`\n❃ اكتب الامر مع عدد التكرار بالرد ؏ النص او او صورة ملصق ليقوم بتكراره مع العدد الذي وضعته\n\n ◂  `.تكرار الملصق` \n❃ بالرد على الملصق ليقوم باستخراج جميع ملصقات الحزمه وارسالها في الدردشة\n\n ◂ `.مكرر`\n❃ اكتب الامر مع وقت بالثواني و مع عدد التكرار وبالرد على صورة او نص  (  تكرار وقتي  )\n\n ◂  `.التكرار`\n❃ بالرد على الرسالة بالامر فقط سيقوم بعمل تكرار سريع ويصل عدده الى 10 الاف تكرار  ! \n\n ◂  `.ايقاف التكرار`\n❃ يقوم هذا الامر بأيقاف التكرار فقط اكتبه\n\n** تنبيه اوامر التكرار قد تؤدي الى حظر حسابك على التلي اذا استخدمتها بشكل غير صحيح**\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م13")
async def hi(event):
    await event.reply(
        "** قائـمه اوامر الأنتحال و التقليد **:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.انتحال`\n❃ بالرد على المستخدم لأنتحال حسابه  من اسم وصورة وبايو  . \n\n ◂  `.اعادة `\n❃ لأعادة حسابك الى وضعه السابق  .\n\n ◂ `.تقليد`\n❃ بالرد على الشخص لتقليد جميع رسائله في الدردشه \n\n ◂ `.الغاء تقليد`\n❃ بالرد على الشخص لايقاف التقليد\n\n ◂ `.المقلدهم `\n❃ لاظهار قائمه الاشخاص الذي فعلت عليهم امر التقليد ولمسحهم ارسل  `.حذف المقلدهم`\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م14")
async def hi(event):
    await event.reply(
        "**قائـمه اوامر الوقتي: **\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯—————𓍻\n◂ `.اسم وقتي`\n❃ بكتابة الامر لاضافة اسم وقتي حسب منطقة التي وضعتها \n\n ◂ `.انهاء اسم وقتي`\n❃ لانهاء الاسم الوقتي و ارجاع الاسم الطبيعي .\n\n ◂ `.بايو وقتي`\n❃ بكتابة الامر لاضافة ساعه يتحرك مع النبذة الخاصة بك  \n\n ◂ `.انهاء بايو وقتي`\n❃ لانهاء البايو الوقتي و ارجاع البايو الطبيعي\n\n ◂  `.الصورة الوقتية`\n❃ لبدء تشغيل وقت على الصورة الخاصة بحسابك\n\n ◂  `.انهاء الصورة الوقتية`\n❃ لألغاء امر الصورة الوقتية\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م15")
async def hi(event):
    await event.reply(
        "**اوامر جلب الذاتية وامر الاضافة**:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.ضيف`\n❃ اكتب الامر مع رابط مجموعه التي تريد سحب اعضائها وارسل الامر في مجموعتك لسحبهم الى مجموعتك\n\n ◂ `.جلب الصورة`  او  `.ذاتية`\n❃ بالرد على صورة او فيديو ذاتية التدمير لحفظها وارسالها في الرسائل المحفوظة بسرية تامة الامر لسورس كرستين حصريا!\n𓍹—————⟮ ᥉ρᎥժᥱᖇ ⟯—————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م16")
async def hi(event):
    await event.reply(
        "قائـمه اوامر البروفايل:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.تغيير اسم`\n❃ لتغيير اسم حسابك اكتب الاسم مع الامر\n\n ◂  `.تغيير بايو`\n❃ لتغيير بايو حسابك اكتب البايو مع الامر \n\n ◂  `.تغيير صورة`\n❃ لتغيير صورة حسابك بالرد على الصورة بالامر\n\n ◂  `.تغيير معرف`\n❃ لتغيير معرف حسابك اكتب المعرف مع الامر\n\n ◂  `.ازالة الصورة`\n❃ اكبت الامر لحذف صورة حسابك\n\n ◂  `.معرفاتي`\n❃ لعرض معرفات حسابك التي صنعتها\n\n ◂  `.حسابي`\n❃ لعرض معلومات و احصائيات حسابك\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م17")
async def hi(event):
    await event.reply(
        "قائـمه اوامر الأكسترا:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.فك المحظورين`\n❃ قم بكتابة الامر لفك جميع المستخدمين الذي حظرتهم من الخاص\n\n ◂  `.وهمي كتابه`\n❃ لبدء وضع كتابة بشكل وهمي جربه بنفسك\n\n ◂  `.وهمي فيديو`\n❃ لبدء وضع ارسال فيديو بشكل وهمي جربه بنفسك\n\n ◂  `.وهمي لعبه`\n❃ لبدء وضع ارسال لعبة بشكل وهمي جربه بنفسك\n\n ◂  `.وهمي صوتي`\n❃ لبدء وضع ارسال بصمة صوتية بشكل وهمي جربه بنفسك\n\n ◂  `.الحاسبة`\n❃ لعرض حاسبه علميه جربه بنفسك\n\n ◂  `.حالتي`\n❃ لعرض حاله حسابك الحاليه اذا كان محظور او لا\n\n ◂  `.صلاة`❃ اكتب الامر مع اسم محافظتك او مدينتك بالانكليزي لعرض اوقات الصلاة والامساك\n\n ◂  `.تغميق`\n❃ بالرد على الكلام لجعله بشكل غامق\n\n ◂  `.نسخ`\n❃ بالرد على الكلام لجعله بشكل للنسخ\n\n ◂  `.مائل`\n❃ بالرد على الكلام لجعله بشكل مائل\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م18")
async def hi(event):
    await event.reply(
        "قائـمه اوامر التاك والملفات:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  او  `.تاك للكل`\n❃ لعمل تاك لاخر 100 عضو في المجموعه اكتب الامر مع اي نص تريد\n\n ◂  `.تبليغ`\n❃ لتبليغ المشرفين اذا حصل شيء ما فقط اكتب الامر\n\n ◂  `.منشن`\n❃ بالرد على المستخدم وكتابه شيء مع الامر لعمل تاك داخل الكلمه للمستخدم\n\n ◂  `.تنصيب`\n❃ لتنصيب ملفات خارجيه للسورس \n\n ◂  `.الغاء تنصيب`\n❃ بكتابه الامر مع اسم الملف لحذف تنصيبه من السورس\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م19")
async def hi(event):
    await event.reply(
        "قائـمه اوامر الصيغ:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n ◂  `.فاس`\n❃ بالرد على متحركو او فيديو لتحويله الى ملصق متحرك\n\n ◂  `.تحويل صورة`\n❃ بالرد على ملصق اتحويله الى صورة\n\n ◂  `.تحويل ملصق`\n❃ بالرد على الصورة لتحويلها الى ملصق\n\n ◂  `.تحويل متحركة `\n❃ بالرد على الفيديو لتحويله الى متحركة \n\n◂  `.لملف`\n❃ بكتابة اسم الملف والرد على كتابة لتحويلها الى ملف\n\n ◂  `.لكتابة`\n❃ بالرد على الملف لاستخراج النص الذي بداخله وارساله لك\n\n ◂  `.الملف لصورة`\n❃ بالرد على الصورة التي تكون بشكل ملف لتحويلها لصورة عادية\n\n ◂  `.تحويل بصمة`\n❃ بالرد على المقطع الوصتي لتحويله الى بصمة\n\n ◂  `.تحويل صوتي`\n❃ بالرد على البصمة لتحويلها على شكل مقطع صوتي\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20",
        link_preview=False,
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م20")
async def hi(event):
    await event.reply(
        "**⌔∮ قائـمة اوامر التسلية**:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n`.غبي`  `.القنابل`  `.بوت`   `.قتل`    `.طوبة`\n\n`.مربعات`   `.حلويات`     `.نار`     `.قمرات`\n\n`.افكر`    `.متت`    `.ضايج`    `.ساعة`\n\n`.مح`    `.قلب`     `.جيم`     `.الارض`\n\n`.قمر`      `.اقمار`     `.قمور `    `.نجمه`\n\n`.مكعبات`   `.مطر `     `.تفريغ`      `.فليم`\n\n`.احبك`    `.طائره`        `.شرطه `\n\n`.ملل`    `.قاتل`       `.عين`\n\n`.افكرر`      `.افعى`         `.وصلت`      `.مايكرو`\n\n`.ادفع`    `.قطار`      `.ادفع `\n\n`.شغل`   `.تحميل`     `.مربع`       `.دائره`\n\n`.انيم`    `.بشره`      `.قرد`      `.يد`\n\n`.العد التنازلي`        `.قلوب`\n\n`.فصخ + الكلام`\n\n`.طفي`     `.تهكير2`   `.تهكير3` .مدينه \n\n .خد .قول هلا .تف .هلو .مصه .اكل  .tf واحد\n\nٴ╼──────────────────╾\n • جميع الاوامر تستخدم بالضغط على الامر راح ينسخ وحده وارسله فقط\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20",
        link_preview=False,
    )
    await event.delete()


@sbb_b.ar_cmd(pattern="م21")
async def hi(event):
    await event.reply(
        "قائمة اوامر سورس سبايدر:\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n- فقط ارسل `.بوتي` بعدها ادخل الى البوت واختر قائمة الفارات.\n\n `.تمبلريه`\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20",
        link_preview=False,
    )
    await event.delete()
    
    
@sbb_b.ar_cmd(pattern="م22")
async def hi(event):
    await event.reply(
        "شـرح عـن اوامـر التنظيـف\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯—————𓍻\n ᯽︙ اختر احدى هذه الاوامر\n\n- ( `.تنظيف + عدد الرسائل` )\nيقوم بحذف الرسائل اكتب الامر وعدد معين من الرسائل سيقوم بحذفها \n\n- ( .تنظيف  + الاضافة )\n يـجب وضع الشـارحه مع الاضافة (-)\nمثـال  :  ( `.تنظيف -ح` )  <سيقوم بحذف المتحركات في الدردشة>\nالاضافات : \n (-ب) : لحـذف الرسائل الـصوتية\n (-م): لحـذف الملفات\n (-ح): لحـذف المتحـركه\n (-ص): لحـذف الـصور\n (-غ): لحـذف الاغاني\n (-ق): لحـذف الـملصقات\n (-ر): لحـذف الـروابط \n(-ف): لحـذف الفـيديوهـات\n\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n᯽︙ CH : @EE_20"
    )
    await event.delete()   


@sbb_b.ar_cmd(pattern="م23")
async def hi(event):
    await event.reply(
        "شـرح عـن اوامـر المسـح\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯—————𓍻\n᯽︙ اختر احدى هذه الاوامر\n\n- ( `.مسح  + بالرد على النص` )\nفقط اكتب الامر بالرد على الرسالة ليقوم بحذفها \n\n- ( `.حذف رسائلي` )\nاكتب الامر في اي مكان وسيقوم بحذف جميع رسائلك في الدردشه حتى لو لم يكن لديك صلاحيات \n\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n᯽︙ CH : @EE_20"
    )
    await event.delete()   
    
  
@sbb_b.ar_cmd(pattern="م24")
async def hi(event):
    await event.reply(
       "**[ 𝙏𝞝𝙇𝞝𝙃𝙊𝙉 𝙎𝙋𝙄𝘿𝞝𝙍 ](https://t.me/EE_20)**\n✦┅━╍━╍╍━━╍━━╍━┅✦\n\n• اوامر حماية المجموعات بالمسح والتحذير\n\n ◂  `.قفل/فتح`\nالسب\nالفارسية\nالميديا\nالبوتات\nالدخول\nالاضافة\nالتوجيه\nالروابط\nالمعرفات\nالكل\n\n• فقط عليك كتابة قفل او فتح مع الاضافة لقفلها ولأظهار الصلاحيات ارسل `.الاعدادات`\n\n ◂  `.البوتات`\n•لمعرفة البوتات في المجموعة ولطردهم" 
    )
    await event.delete()      
    
    
@sbb_b.ar_cmd(pattern="م25")
async def hi(event):
    await event.reply(
        "᯽︙ اوامر الهمسه واكس او \n\n⌔︙الامر  • `.همسة`\n⌔︙الاستخدام  • لكتابة همسه سرية لشخص في المجموعه \n\n᯽︙ الامر • `.الهمسة`\n᯽︙ استخدامه • لعرض كيفية كتابة همسة سرية\n\n᯽︙ الامر • `.اكس او `\n ᯽︙ استخدامه • ففط ارسل الامر لبدء لعبة اكس او\n\n᯽︙ CH  - @EE_20"
    )        
    await event.delete()         


@sbb_b.ar_cmd(pattern="م26")
async def hi(event):
    await event.reply(
        " قائمة اوامر بصمات وميمز الانمي\n 𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n᯽︙ اختر احدى هذه الاوامر\n\n- ( `.انمي1` )\n- ( `.انمي2` )\n- ( `.انمي3` )\n- ( `.انمي4` )\n- ( `.انمي5` )\n- ( `.انمي6` )\n- ( `.انمي7` )\n- ( `.انمي8` )\n- ( `.انمي9` )\n- ( `.انمي10` )\n - ( `.اجمد كده` )\n- ( `.لماذا نحن هنا ` )\n- ( `.بووم` )\n- ( `.خلصانه` )\n- ( `.تعبان` )\n- ( `.يلاهوي` )\n- ( `.انا رمضان جانا` )\n- ( `.يا حلو` )\n- ( `.عاوزين نعملك بني ادم` )\n- ( `.ربنا نصرني ` )\n - ( `.اتكلم بادب ` )\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )        
    await event.delete()             
    
    
@sbb_b.ar_cmd(pattern="م27")
async def hi(event):
    await event.reply(
        "شرح اوامر الروابط :\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n᯽︙ اختر احدى هذه الاوامر\n\n- ( `.دنس` + الرابط )\nلكشف نظام دومين موقع معين اكتب الامر مع الرابط\n\n- ( `.مصغر` )\nبالرد على الرابط او وضع الرابط مع الامر ليقوم بتصغيره\n\n- ( `.اخفاء` )\nبالرد على الرابط لاخفاء الرابط و جعله في مسافه معينة جرب الامر بنفسك\n\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )        
    await event.delete()

@sbb_b.ar_cmd(pattern="م30")
async def hi(event):
    await event.reply(
        " اضافات سورس سبايدر :\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n᯽︙ اختر احدى هذه الاوامر\n\n⎊ ( `.الصيد` )\nلعرض اوامر صيد اليوزرات\n\n⎊ ( `.تاك` )\nلعمل تاك لأعضاء الجروب او القناة\n\n⎊ تيلثون سبايدر الذكاء الاصطناعي 🌐\n⎊ اكتب `.os` + اي شي تيريد البحث عنه \n⎊ مثال `.os كم عدد سكان مصر`\n\n⎊ لعرض العاب سورس سبايدر اكتب `.الالعاب` \n⎊ لبدأ لعبة XO اكتب `.اكس او`\n⎊ اكتب `.شطرنج` لبدأ لعبه الشطرنج\n⎊ لالعاب الاسئله اكتب (`.كت` او `.خيروك`) \n⎊ للاذكار اكتب (`.اذكار`)\n⎊ لجلب معلومات عن حساب github\n⎊ اكتب ( `.كيثهاب` + اليوزر )n\n⎊ سورس سبايدر اوامر المكالمه الصوتية \n⎊ اكتب `.بدء مكالمه` لبدء مكالمة صوتيه \n⎊ لدعوة شخص اكتب `.دعوه للمكالمه` + اليوزر\n\n⎊ سورس سبايدر فحص فيزات 💳\n⎊ لجلب فيزات اكتب (`.فيزا`) \n⎊ لفحص فيزا اعمل ريب على الفيزا واكتب `.cc`\n\n⎊ سورس سبايدر للبحث عن التطبيقات 🎲\n⎊ ( اكتب `.تطبيق` + اسم التطبيق ) \n\n⎊ سورس سبايدر للبحث عن الصور ✨\n⎊ ( اكتب `.صورة` + اسم الصور ) \n\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )        
    await event.delete()            
    
    
@sbb_b.ar_cmd(pattern="م29")
async def hi(event):
    await event.reply(
        "اوامر تجميع نقاط تمويل :\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n᯽︙ اختر احـدي هذه الاوامر\n\n- ( `.تجميع النقاط` )\nلتجميع نقاط بوت تمويل المليار @t06bot \n\n- ( `.تجميع الجوكر` )\nلتجميع نقاط بوت تمويل الجوكر @A_MAN9300BOT \n\n- ( `.تجميع العملاق` )\nلتجميع نقاط بوت تمويل العملاق @TMWEL10M_BOT \n\n- ( `.تجميع العقاب` )\nلتجميع نقاط بوت تمويل العقاب @MARKTEBOT \n\n- ( `.تجميع العرب` )\nلتجميع نقاط بوت تمويل العرب @xnsex21bot \n\n- ( `.تجميع تيربو` )\nلتجميع نقاط بوت تمويل تيربو @TURBO7xbot \n\n𓍹——————⟮ ᥉ρᎥժᥱᖇ ⟯——————𓍻\n⌔︙CH : @EE_20"
    )        
    await event.delete()            
    
    
@sbb_b.ar_cmd(pattern="م28")
async def we(event):
    await edit_or_reply(
        event,
        "**[ 𝙏𝞝𝙇𝞝𝙃𝙊𝙉 𝙎𝙋𝙄𝘿𝞝𝙍 ](https://t.me/EE_20)**\n✦┅━╍━╍╍━━╍━━╍━┅✦\n\n• اوامر التحكم تتيح لمستخدم اخر ان يستخدم اوامر تنصيبك وهو غير منصب بوت سبايدر حيث سيصبح متحكم في اوامر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامر وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.التحكم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب بوت سبايدر الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين"
    ) 
    await event.delete()
    
    
    
    
   
